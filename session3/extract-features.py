...
def extract_features_prep(tree, entities, e1, e2):
    ...
    if tkE1 is not None and tkE2 is not None:
        ...
        prepositions = []
        for tk in range(tkE1 + 1, tkE2):
            if tree.is_preposition(tk):
                prepositions.append(tree.get_word(tk).lower())
...
#         for tk in range(tkE1 + 1, tkE2):
#             if tree.is_verb(tk):
#                 verbs.append(tree.get_word(tk).lower())
...
#         # Add entity types
#         e1_type = entities[e1]['type']
#         e2_type = entities[e2]['type']
...
#             next_entity_types = []
#             for tk in range(entity + 1, min(len(tree.tokens), entity + 3)): #two
#                 if tree.is_entity(tk, entities):
#                     next_entity_id = tree.get_entity_id(tk, entities)
#                     next_entity_type = entities[next_entity_id]['type']
...
#datadir = sys.argv[1]
datadir = "C:/Users/cperez/OneDrive - IREC-edu/05_MASTER/MUD/lab_resources/lab_resources/DDI/data/train"
...
for f in listdir(datadir) :
    ...
    for s in sentences :
        ...
        for p in pairs:
            ...
            feats = extract_features_prep(analysis,entities,id_e1,id_e2) 
            # resulting vector
            if len(feats) != 0:
